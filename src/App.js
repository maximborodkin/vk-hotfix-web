			...
			<Switch>
				...
				<Route path="/basket/:areaId/:itemId" exact>
					<Basket
						foodAreas={FOOD_AREAS}
						order={order}
						onOrderDetailsEdited = {( { id, newOrderSettings }) => {
							const updatedOrderSettings = {...orderSettings};

							if (id in updatedOrderSettings) {
								updatedOrderSettings[id].fastAsCan = newOrderSettings.fastAsCan;
								updatedOrderSettings[id].time = newOrderSettings.time;
								updatedOrderSettings[id].selfService = newOrderSettings.selfService;
							} else {
								updatedOrderSettings[id] = {
									fastAsCan: newOrderSettings.fastAsCan,
									time: newOrderSettings.time,
									selfService: newOrderSettings.selfService
								};
							}
							const serialized = JSON.stringify(updatedOrderSettings)
							localStorage.setItem('orderSettings', serialized);
						} }
					/>
				</Route>
				<Route
