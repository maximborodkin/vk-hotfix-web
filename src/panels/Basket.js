...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order, onOrderDetailsEdited }) => {
  ...
  const [ price, products ] = useMemo(() => {
    ...
    if (localStorage.getItem('orderSettings')){
      let data = JSON.parse(localStorage.getItem('orderSettings'))
            if (faster !== data[item.id].fastAsCan) setFaster(data[item.id].fastAsCan)
            if (time !== data[item.id].time) setTime(data[item.id].time)
            if (selfService !== data[item.id].selfService) setSelfService(data[item.id].selfService)
      }
    ...
    <div className="Place">
      ...
      <footer className="Place__footer">
        <Link
            ...
            onClick={() => {
              onOrderDetailsEdited({ id: item.id, newOrderSettings: {fastAsCan: faster, time: time, selfService: selfService} })
            }}
